...
import { DEVICE_MODULE_NAME, type DeviceModule } from './native.generated.js';

/** Resolve this package's native module without a central runtime registry. */
export function requireNativeModule(): DeviceModule {
  'background only';
  const nativeModule = NativeModules[DEVICE_MODULE_NAME] as
    | DeviceModule
    | null
    | undefined;
  if (nativeModule === undefined || nativeModule === null) {
    throw new Error('Device is not registered by the host');
  }
  return nativeModule;
}
    ...
    try {
      action((error) => {
        ...
        if (typeof error !== 'string') {
          reject(new Error('Device returned an invalid error value'));
          return;
        }
        if (error.length > 0) {

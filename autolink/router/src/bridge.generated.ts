...
import { ROUTER_MODULE_NAME, type RouterModule } from './native.generated.js';

/** Resolve this package's native module without a central runtime registry. */
export function requireNativeModule(): RouterModule {
  'background only';
  const nativeModule = NativeModules[ROUTER_MODULE_NAME] as
    | RouterModule
    | null
    | undefined;
    ...
    try {
      action((error) => {
        ...
        if (typeof error !== 'string') {
          reject(new Error('Router returned an invalid error value'));
          return;
        }
        if (error.length > 0) {

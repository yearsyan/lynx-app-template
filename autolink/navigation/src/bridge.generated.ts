...
export function requireNativeModule(): NavigationModule {
  'background only';
  const nativeModule = NativeModules[NAVIGATION_MODULE_NAME] as
    | NavigationModule
    | null
    | undefined;
  if (nativeModule === undefined || nativeModule === null) {
    throw new Error('Navigation is not registered by the host');
  }
  return nativeModule;
}
    ...
    try {
      action((error) => {
        ...
        if (typeof error !== 'string') {
          reject(new Error('Navigation returned an invalid error value'));
          return;
        }
        if (error.length > 0) {

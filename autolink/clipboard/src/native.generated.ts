// Generated from contracts/native-modules.json. Do not edit.
import '@lynx-app/native-runtime';
import type { Clipboard as RawClipboardModule } from '../types/platform-native-module.js';

export type ClipboardModule = RawClipboardModule;

declare module '@lynx-app/native-runtime' {
  interface NativeModuleRegistry {
    Clipboard?: RawClipboardModule;
  }
}

/** Name the native hosts register this module under. */
export const CLIPBOARD_MODULE_NAME = 'Clipboard' as const;

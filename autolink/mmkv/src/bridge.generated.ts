...
import { KV_MODULE_NAME, type KVModule } from './native.generated.js';

/** Resolve this package's native module without a central runtime registry. */
export function requireNativeModule(): KVModule {
  'background only';
  const nativeModule = NativeModules[KV_MODULE_NAME] as
    | KVModule
    | null
    | undefined;
    ...
    try {
      action((error) => {
        ...
        if (typeof error !== 'string') {
          reject(new Error('KV returned an invalid error value'));
          return;
        }
        if (error.length > 0) {

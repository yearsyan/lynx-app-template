...
import { SENSORS_MODULE_NAME, type SensorsModule } from './native.generated.js';

/** Resolve this package's native module without a central runtime registry. */
export function requireNativeModule(): SensorsModule {
  'background only';
  const nativeModule = NativeModules[SENSORS_MODULE_NAME] as
    | SensorsModule
    | null
    | undefined;
  if (nativeModule === undefined || nativeModule === null) {
    throw new Error('Sensors is not registered by the host');
  }
  return nativeModule;
}
    ...
    try {
      action((error) => {
        ...
        if (typeof error !== 'string') {
          reject(new Error('Sensors returned an invalid error value'));
          return;
        }
        if (error.length > 0) {

...
import { SCANNER_MODULE_NAME, type ScannerModule } from './native.generated.js';

/** Resolve this package's native module without a central runtime registry. */
export function requireNativeModule(): ScannerModule {
  'background only';
  const nativeModule = NativeModules[SCANNER_MODULE_NAME] as
    | ScannerModule
    | null
    | undefined;
  if (nativeModule === undefined || nativeModule === null) {
    throw new Error('Scanner is not registered by the host');
  }
  return nativeModule;
}

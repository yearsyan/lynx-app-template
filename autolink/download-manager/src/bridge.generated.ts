...
import {
  DOWNLOAD_MANAGER_MODULE_NAME,
  type DownloadManagerModule,
} from './native.generated.js';

/** Resolve this package's native module without a central runtime registry. */
export function requireNativeModule(): DownloadManagerModule {
  'background only';
  const nativeModule = NativeModules[DOWNLOAD_MANAGER_MODULE_NAME] as
    | DownloadManagerModule
    | null
    | undefined;
  if (nativeModule === undefined || nativeModule === null) {
    throw new Error('DownloadManager is not registered by the host');
  }
  return nativeModule;
}

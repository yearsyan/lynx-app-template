...
import { HAPTICS_MODULE_NAME, type HapticsModule } from './native.generated.js';

/** Resolve this package's native module without a central runtime registry. */
export function requireNativeModule(): HapticsModule {
  'background only';
  const nativeModule = NativeModules[HAPTICS_MODULE_NAME] as
    | HapticsModule
    | null
    | undefined;
    ...
    try {
      action((error) => {
        ...
        if (typeof error !== 'string') {
          reject(new Error('Haptics returned an invalid error value'));
          return;
        }
        if (error.length > 0) {

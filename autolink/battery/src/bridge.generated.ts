...
import { BATTERY_MODULE_NAME, type BatteryModule } from './native.generated.js';

/** Resolve this package's native module without a central runtime registry. */
export function requireNativeModule(): BatteryModule {
  'background only';
  const nativeModule = NativeModules[BATTERY_MODULE_NAME] as
    | BatteryModule
    | null
    | undefined;
  if (nativeModule === undefined || nativeModule === null) {
    throw new Error('Battery is not registered by the host');
  }
  return nativeModule;
}

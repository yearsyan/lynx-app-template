...
import {
  NETWORK_INFO_MODULE_NAME,
  type NetworkInfoModule,
} from './native.generated.js';

/** Resolve this package's native module without a central runtime registry. */
export function requireNativeModule(): NetworkInfoModule {
  'background only';
  const nativeModule = NativeModules[NETWORK_INFO_MODULE_NAME] as
    | NetworkInfoModule
    | null
    | undefined;
  if (nativeModule === undefined || nativeModule === null) {
    throw new Error('NetworkInfo is not registered by the host');
  }
  return nativeModule;
}
    ...
    try {
      action((error) => {
        ...
        if (typeof error !== 'string') {
          reject(new Error('NetworkInfo returned an invalid error value'));
          return;
        }
        if (error.length > 0) {

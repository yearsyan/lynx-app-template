...
import { SHARE_MODULE_NAME, type ShareModule } from './native.generated.js';

/** Resolve this package's native module without a central runtime registry. */
export function requireNativeModule(): ShareModule {
  'background only';
  const nativeModule = NativeModules[SHARE_MODULE_NAME] as
    | ShareModule
    | null
    | undefined;
  if (nativeModule === undefined || nativeModule === null) {
    throw new Error('Share is not registered by the host');
  }
  return nativeModule;
}

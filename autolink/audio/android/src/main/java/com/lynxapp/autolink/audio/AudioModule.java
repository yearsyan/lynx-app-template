package com.lynxapp.autolink.audio;
...
/**
 * Local-file audio playback backed by MediaPlayer plus microphone recording
 * backed by MediaRecorder. Every operation is serialized onto the main
 * looper (MediaPlayer and MediaRecorder need one) and events flow back to
 * JS as `audioPlayer` / `audioRecorder` global events. Audio focus is
 * managed per player according to its usage; ambient players never take
 * focus. Recording requires the RECORD_AUDIO runtime permission to be
 * granted already (the Permissions module owns the prompt).
 */
@LynxNativeModule(name = AudioModule.NAME)
public final class AudioModule extends LynxContextModule {
    public static final String NAME = "Audio";
    public static final String EVENT_NAME = "audioPlayer";
    public static final String RECORDER_EVENT_NAME = "audioRecorder";
    ...
    private static final String REC_STATE_IDLE = "idle";
    private static final String REC_STATE_RECORDING = "recording";
    private static final String REC_STATE_PAUSED = "paused";
    private static final String REC_STATE_STOPPED = "stopped";
    private static final String REC_STATE_FAILED = "failed";
    ...
    private final Map<String, RecorderHandle> recorders = new ConcurrentHashMap<>();
    private final AudioManager audioManager;
    private final Handler mainHandler = new Handler(Looper.getMainLooper());
    private volatile boolean destroyed = false;

    public AudioModule(LynxContext context) {
        super(context);
    ...
    @LynxMethod
    public void recorderCreate(ReadableMap options, Callback callback) {
        String id = orEmpty(options.hasKey("id") ? options.getString("id") : null);
        int durationLimitMs = options.hasKey("durationLimitMs") && !options.isNull("durationLimitMs")
                ? options.getInt("durationLimitMs")
                : 0;
        int progressIntervalMs = options.hasKey("progressIntervalMs") && !options.isNull("progressIntervalMs")
                ? options.getInt("progressIntervalMs")
                : DEFAULT_PROGRESS_INTERVAL_MS;
        ...
        if (destroyed) {
            callback.invoke("Audio host has been destroyed");
            return;
        }
        if (!PLAYER_ID.matcher(id).matches()) {
            callback.invoke("Invalid AudioRecorder ID");
            return;
        }
        if (durationLimitMs != 0 && (durationLimitMs < 100 || durationLimitMs > 600_000)) {
            callback.invoke("durationLimitMs must be between 100 and 600000, or 0 to disable");
            return;
        }
        if (progressIntervalMs < 50 || progressIntervalMs > 10_000) {
            callback.invoke("progressIntervalMs must be between 50 and 10000");
        ...
        mainHandler.post(() -> {
            ...
            if (recorders.containsKey(id)) {
                callback.invoke("AudioRecorder ID already exists");
                return;
            }
            RecorderHandle handle = new RecorderHandle(id, durationLimitMs, progressIntervalMs);
            recorders.put(id, handle);
            callback.invoke("");
        });
    }

    @LynxMethod
    public void recorderStart(String id, Callback callback) {
        withRecorder(id, callback, handle -> handle.start(callback));
    }

    @LynxMethod
    public void recorderPause(String id, Callback callback) {
        withRecorder(id, callback, handle -> handle.pause(callback));
    }

    @LynxMethod
    public void recorderResume(String id, Callback callback) {
        withRecorder(id, callback, handle -> handle.resume(callback));
    }

    @LynxMethod
    public void recorderStop(String id, Callback callback) {
        withRecorder(id, callback, handle -> handle.stop(callback));
    }

    @LynxMethod
    public void recorderCancel(String id, Callback callback) {
        mainHandler.post(() -> {
            RecorderHandle handle = recorders.get(id);
            if (handle == null) {
                callback.invoke("Unknown AudioRecorder ID");
                return;
            }
            recorders.remove(id);
            handle.cancel();
    ...
    @LynxMethod
    public void recorderGetProps(String id, Callback callback) {
        withRecorder(id, callback, handle -> handle.getProps(callback));
    }

    @LynxMethod
    public void recorderRelease(String id, Callback callback) {
        mainHandler.post(() -> {
            RecorderHandle handle = recorders.get(id);
            if (handle == null) {
                callback.invoke("Unknown AudioRecorder ID");
    ...
    public void destroy() {
        ...
        mainHandler.post(() -> {
            ...
            players.clear();
            for (RecorderHandle handle : recorders.values()) {
                handle.release();
            }
            recorders.clear();
        });
    ...
    private interface RecorderOperation {
        void run(RecorderHandle handle);
    }

    private void withRecorder(String id, Callback callback, RecorderOperation operation) {
        mainHandler.post(() -> {
            RecorderHandle handle = recorders.get(id);
            if (handle == null) {
                callback.invoke("Unknown AudioRecorder ID");
    ...
    private void emit(String id, JavaOnlyMap payload) {
        emitEvent(EVENT_NAME, id, payload);
    }

    private void emitRecorder(String id, JavaOnlyMap payload) {
        emitEvent(RECORDER_EVENT_NAME, id, payload);
    }

    private void emitEvent(String eventName, String id, JavaOnlyMap payload) {
        if (destroyed) {
            return;
        }
        ...
        mainHandler.post(() -> {
            ...
            if (context != null) {
                context.sendGlobalEvent(eventName, JavaOnlyArray.of(payload));
            }
        });
    }
    ...
    /**
     * Owns one MediaRecorder writing AAC audio to the cache directory. All
     * methods run on the main looper; the recorded duration is tracked from
     * elapsedRealtime because MediaRecorder reports none.
     */
    private final class RecorderHandle {
        final String id;
        ...
        File outputFile;
        String state = REC_STATE_IDLE;
        long recordedMs = 0L;
        long segmentStartAt = 0L;
        boolean delivered = false;
        @Nullable String deliveredUri;
        long deliveredDurationMs;
        @Nullable String failure;
        private final Runnable progressTicker = new Runnable() {
            @Override
            public void run() {
                if (REC_STATE_RECORDING.equals(state)) {
                    emitProgress();
                    if (durationLimitMs > 0 && durationMs() >= durationLimitMs) {
                        stop(null);
                        return;
                    }
                    mainHandler.postDelayed(this, progressIntervalMs);
        ...
        RecorderHandle(String id, int durationLimitMs, int progressIntervalMs) {
            this.id = id;
        ...
        void start(Callback callback) {
            if (!REC_STATE_IDLE.equals(state) && !REC_STATE_STOPPED.equals(state)) {
                callback.invoke("AudioRecorder has already been started");
                return;
            }
            ...
            File file;
            try {
                file = recordingFile(context, id);
            } catch (IOException error) {
                callback.invoke(orEmpty(error.getMessage()).isEmpty()
                        ? "Unable to prepare the recording file"
                        : error.getMessage());
                return;
            }
            if (REC_STATE_STOPPED.equals(state)) {
                // A second take replaces the previous file entirely.
                recordedMs = 0L;
                delivered = false;
                deliveredUri = null;
            }
            MediaRecorder newRecorder = buildRecorder(context, file);
            try {
                newRecorder.prepare();
                newRecorder.start();
            } catch (Throwable error) {
                ...
                deleteIfExists(file);
                callback.invoke("Unable to start the recording: "
                        + orEmpty(error.getMessage()));
                return;
            }
            recorder = newRecorder;
            outputFile = file;
            state = REC_STATE_RECORDING;
            segmentStartAt = SystemClock.elapsedRealtime();
            emitState(REC_STATE_RECORDING);
            mainHandler.postDelayed(progressTicker, progressIntervalMs);
            callback.invoke("");
        }
        ...
        void pause(Callback callback) {
            if (!REC_STATE_RECORDING.equals(state)) {
                callback.invoke(REC_STATE_IDLE.equals(state)
                        ? "AudioRecorder has not been started"
                        : "");
                return;
            }
            mainHandler.removeCallbacks(progressTicker);
            recordedMs += SystemClock.elapsedRealtime() - segmentStartAt;
            try {
                recorder.pause();
            } catch (Throwable error) {
                fail(orEmpty(error.getMessage()).isEmpty()
                        ? "Unable to pause the recording"
                        : error.getMessage());
                callback.invoke(failure);
                return;
            }
            state = REC_STATE_PAUSED;
            emitState(REC_STATE_PAUSED);
            callback.invoke("");
        }

        void resume(Callback callback) {
            if (!REC_STATE_PAUSED.equals(state)) {
                callback.invoke(REC_STATE_IDLE.equals(state)
                        ? "AudioRecorder has not been started"
                        : "");
                return;
        ...
        void stop(@Nullable Callback callback) {
            if (!REC_STATE_RECORDING.equals(state) && !REC_STATE_PAUSED.equals(state)) {
                if (callback != null) {
                    callback.invoke(delivered ? resultJSON() : stopError());
                }
                return;
            }
            mainHandler.removeCallbacks(progressTicker);
            ...
            try {
                ...
                // stop() throws when nothing was captured; the file is unusable.
                releaseRecorder();
                deleteIfExists(outputFile);
                outputFile = null;
                state = REC_STATE_FAILED;
                failure = "no audio was captured: " + orEmpty(error.getMessage());
                emitState(REC_STATE_FAILED);
                emitError(failure);
                if (callback != null) {
                    callback.invoke(failure);
                }
            ...
            delivered = true;
            deliveredUri = Uri.fromFile(outputFile).toString();
            deliveredDurationMs = recordedMs;
            emitState(REC_STATE_STOPPED);
            if (callback == null) {
                // Auto stop triggered by the duration limit: deliver through
                // the end event, matching an explicit stop's payload.
                JavaOnlyMap end = new JavaOnlyMap();
                end.putString("type", "end");
                end.putString("uri", deliveredUri);
                end.putDouble("durationMs", deliveredDurationMs);
                emitRecorder(id, end);
            } else {
        ...
        void getProps(Callback callback) {
            ...
            try {
                props.put("state", state);
                props.put("durationMs", durationMs());
                props.put("uri", deliveredUri == null ? JSONObject.NULL : deliveredUri);
                callback.invoke(props.toString());
            } catch (JSONException error) {
                callback.invoke("{\"error\":\"AudioRecorder serialization failed\"}");
            }
        }
        ...
        private MediaRecorder buildRecorder(Context context, File file) {
            MediaRecorder newRecorder;
            if (android.os.Build.VERSION.SDK_INT >= 31) {
                newRecorder = new MediaRecorder(context);
            } else {
            ...
            if (durationLimitMs > 0) {
                // Also enforced natively as a hard cap; the ticker usually
                // stops first with a cleaner state sequence.
                newRecorder.setMaxDuration(durationLimitMs);
            }
            newRecorder.setOutputFile(file.getAbsolutePath());
            newRecorder.setOnErrorListener((mr, what, extra) -> {
                if (REC_STATE_RECORDING.equals(state) || REC_STATE_PAUSED.equals(state)) {
                    mainHandler.removeCallbacks(progressTicker);
                    releaseRecorder();
                    state = REC_STATE_FAILED;
                    failure = "Recording failed (" + what + "/" + extra + ")";
                    emitState(REC_STATE_FAILED);
                    emitError(failure);
                }
            });
        ...
        private void releaseRecorder(@Nullable MediaRecorder target) {
            if (target != null) {
                try {
                    target.release();
                } catch (Throwable _error) {
        ...
        private String resultJSON() {
            ...
            try {
                result.put("uri", deliveredUri == null ? JSONObject.NULL : deliveredUri);
                result.put("durationMs", deliveredDurationMs);
                result.put("sizeBytes", outputFile == null || !outputFile.isFile()
                        ? JSONObject.NULL
                        : outputFile.length());
                return result.toString();
            } catch (JSONException error) {
                return "{\"error\":\"AudioRecorder serialization failed\"}";
            }
        }

        private String stopError() {
            return REC_STATE_FAILED.equals(state) && failure != null
                    ? failure
                    : "AudioRecorder has not been started";
        }

        private long durationMs() {
            long extra = REC_STATE_RECORDING.equals(state)
                    ? SystemClock.elapsedRealtime() - segmentStartAt
                    : 0L;
            return recordedMs + extra;
        }
        ...
        private void emitState(String newState) {
            ...
            payload.putString("type", "state");
            payload.putString("state", newState);
            payload.putDouble("durationMs", durationMs());
            emitRecorder(id, payload);
        }

        private void emitProgress() {
            JavaOnlyMap payload = new JavaOnlyMap();
            payload.putString("type", "progress");
            payload.putString("state", REC_STATE_RECORDING);
            payload.putDouble("durationMs", durationMs());
            emitRecorder(id, payload);
        }
        ...
        private Context appContext() {
            Context context = mLynxContext != null ? mLynxContext.getApplicationContext() : null;
            if (context == null) {
                throw new IllegalStateException("Audio module has no host context");
            }
            return context;
        }
    }

    private static File recordingFile(Context context, String id) throws IOException {
        File directory = new File(new File(context.getCacheDir(), "LynxFiles"), "recordings");
        if (!directory.isDirectory() && !directory.mkdirs()) {
            throw new IOException("Unable to create the recordings directory");
        }
        File file = new File(directory, id + ".m4a");
        if (file.exists() && !file.delete()) {
            throw new IOException("Unable to replace the previous recording");
        }
        return file;
    }

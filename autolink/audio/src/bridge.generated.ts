...
import { AUDIO_MODULE_NAME, type AudioModule } from './native.generated.js';

/** Resolve this package's native module without a central runtime registry. */
export function requireNativeModule(): AudioModule {
  'background only';
  const nativeModule = NativeModules[AUDIO_MODULE_NAME] as
    | AudioModule
    | null
    | undefined;
  if (nativeModule === undefined || nativeModule === null) {
    throw new Error('Audio is not registered by the host');
  }
  return nativeModule;
}

...
import {
  APP_INSTALLER_MODULE_NAME,
  type AppInstallerModule,
} from './native.generated.js';

/** Resolve this package's native module without a central runtime registry. */
export function requireNativeModule(): AppInstallerModule {
  'background only';
  const nativeModule = NativeModules[APP_INSTALLER_MODULE_NAME] as
    | AppInstallerModule
    | null
    | undefined;
  if (nativeModule === undefined || nativeModule === null) {
    throw new Error('AppInstaller is not registered by the host');
  }
  return nativeModule;
}
    ...
    try {
      action((error) => {
        ...
        if (typeof error !== 'string') {
          reject(new Error('AppInstaller returned an invalid error value'));
          return;
        }
        if (error.length > 0) {

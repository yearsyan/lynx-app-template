...
import { CAMERA_MODULE_NAME, type CameraModule } from './native.generated.js';

/** Resolve this package's native module without a central runtime registry. */
export function requireNativeModule(): CameraModule {
  'background only';
  const nativeModule = NativeModules[CAMERA_MODULE_NAME] as
    | CameraModule
    | null
    | undefined;
  if (nativeModule === undefined || nativeModule === null) {
    throw new Error('Camera is not registered by the host');
  }
  return nativeModule;
}

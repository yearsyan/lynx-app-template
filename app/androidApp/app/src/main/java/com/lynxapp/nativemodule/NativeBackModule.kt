...
/**
 * Activity-owned back dispatcher shared with the page's NativeBackModule.
 *
...
class NativeBackController(private val activity: Activity) {
    ...
    companion object {
        const val EVENT_NAME = "nativeBack"
...
class NativeBackModule(context: android.content.Context, param: Any?) : LynxModule(context, param) {
    ...
    fun setEnabled(enabled: Boolean, callback: Callback) {
        ...
        if (host == null || backController == null) {
            callback.invoke("Native back has no Activity host")
            return
        }
        host.runOnUiThread {
    ...
    companion object {
        const val NAME = "NativeBackModule"
    }
}

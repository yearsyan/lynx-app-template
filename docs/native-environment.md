...
`lib/native-host` 中的公共实现扩展了 ReactLynx 的 `InitData` 类型，并对缺失、负数和无效值回退为 `0`。所有 bundle 都从 `@lynx-app/native-host` 读取同一份宿主数据契约。页面保持最外层背景全屏，只在内容容器的 padding 上叠加安全区：
...
新增 bundle 只需依赖 `@lynx-app/native-host`，即可复用同一类型与安全区读取函数；字段名称和含义应保持向后兼容。

## 三端数据源

- Android：监听 `WindowInsetsCompat`，合并 system bars 与 display cutout，再换算为 dp；旋转、分屏和系统栏显隐会自动更新。
- iOS：读取 `UIView.safeAreaInsets`，在 `viewSafeAreaInsetsDidChange()` 以及布局变化后更新；单位天然是 point。
- HarmonyOS：监听主窗口 `avoidAreaChange`，合并 system、cutout 与 navigation indicator，使用 `px2vp()` 后更新 `LynxContext`。

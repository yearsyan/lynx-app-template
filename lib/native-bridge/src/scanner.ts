...
function requireScannerModule() {
  'background only';
  return requireNativeModule(NATIVE_MODULE_NAMES.Scanner);
}

function decodeEnvelope(resultJSON: string): ScannerEnvelope {
  'background only';
  if (typeof resultJSON !== 'string') {
    throw new Error('Scanner returned a non-string result');
  }
  const parsed = JSON.parse(resultJSON) as unknown;
  if (typeof parsed !== 'object' || parsed === null) {
    throw new Error('Scanner returned an invalid result');
  }
  return parsed as ScannerEnvelope;
}
  ...
  return new Promise((resolve, reject) => {
    try {
      action((resultJSON) => {
        'background only';
        try {
          const envelope = decodeEnvelope(resultJSON);

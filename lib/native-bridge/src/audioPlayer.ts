...
function requireAudioPlayerModule() {
  'background only';
  return requireNativeModule(NATIVE_MODULE_NAMES.AudioPlayer);
}
...
export class AudioPlayerHandle {
  ...
  getProps(): Promise<AudioPlayerProps> {
    ...
    return new Promise((resolve, reject) => {
      requireAudioPlayerModule().getProps(this.id, (resultJSON) => {
        'background only';
        try {
          if (typeof resultJSON !== 'string') {
            throw new Error('AudioPlayer returned a non-string result');
          }
          const parsed = JSON.parse(resultJSON) as unknown;
          if (typeof parsed !== 'object' || parsed === null) {
            throw new Error('AudioPlayer returned an invalid result');
          }
          const props = parsed as AudioPlayerPropsPayload;
